...
`openCashDrawer()` pops the cash drawer, which is wired through the receipt printer's port rather than being its own USB device, with no paper output — same behavior as RaceFacer, which already kicks this same drawer this way. The exact command was recovered from `window.open_drawer` in RaceFacer's `pos.js` (RaceFacer bundles its own copy of `qz-tray.js`): two raw text macros, `"p\x0022"` and `"p22"`, sent through a QZ config matching RaceFacer's own exactly (`size`/`margins`/`colorType`/`interpolation`/`scaleContent`/`density`) — a bare config (just the printer name, no options) silently did nothing, so the driver on this machine apparently only recognizes these macros as drawer-kick codes through that specific config shape. Only one of the two macros is honored depending on the driver (expected, not an error); an error is only surfaced if both fail. The button is on `/session` (kiosk mode, F9) so a CSR can open the drawer without a supervisor login, and also on `/parametres` for testing.
