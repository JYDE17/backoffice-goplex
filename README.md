...
`openCashDrawer()` pops the cash drawer, which is wired through the receipt printer's port rather than being its own USB device. Neither generic ESC/POS bytes nor RaceFacer's own raw text macros (recovered from `window.open_drawer` in RaceFacer's `pos.js`, which bundles its own `qz-tray.js` and kicks this same drawer successfully) pop it on this machine — only an actual completed print job does, confirmed by the fact that printing a test receipt already pops the drawer every time. The printer/driver here evidently pulses the drawer as a side effect of any finished print job (a common Windows driver "kick drawer after print" setting) rather than reacting to a standalone raw command, so `openCashDrawer()` just calls `printReceiptHtml` with a near-blank receipt to trigger that same pulse. The button is on `/session` (kiosk mode, F9) so a CSR can open the drawer without a supervisor login, and also on `/parametres` for testing.
