...
`openCashDrawer()` pops the cash drawer, which is wired through the receipt printer's port rather than being its own USB device. It first tries RaceFacer's own approach exactly — two raw text macros (`"p\x0022"` and `"p22"`, recovered from `window.open_drawer` in RaceFacer's `pos.js`) sent through the same QZ config (`size`/`margins`/`colorType`/`interpolation`/`scaleContent`/`density`) to the same target printer (`EPSON TM-T88VI Receipt`, confirmed against RaceFacer's own `data-termalprintername` attribute — printer identity was double-checked and isn't the mismatch) — meant to pop the drawer with no paper output, the way RaceFacer does. If both variants reject, it falls back to `printReceiptHtml("")`: a real, near-blank print job, confirmed to reliably pop this drawer as a side effect of the printer driver's own "pulse after print" behavior, at the cost of a small amount of paper feed. Why the raw macros alone don't reproduce RaceFacer's paperless result on this machine is still unresolved (config and printer both match) — the fallback exists so the button stays reliable either way. The button is on `/session` (kiosk mode, F9) so a CSR can open the drawer without a supervisor login, and also on `/parametres` for testing.
