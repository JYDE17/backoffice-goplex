...
`openCashDrawer()` pops the cash drawer, which is wired through the receipt printer's port rather than being its own USB device. Neither generic ESC/POS bytes nor RaceFacer's own raw text macros (recovered from `window.open_drawer` in RaceFacer's `pos.js`, sent through a QZ config matching RaceFacer's own exactly) pop it through QZ's raw print pipeline on this machine — only an actual completed print job does, confirmed by the fact that printing a test receipt already pops the drawer every time. So `openCashDrawer()` calls that same proven `printReceiptHtml` path with genuinely empty content, trading a still-unexplained raw-command failure for a small amount of paper feed from the printer's own cut/feed behavior on every open. Revisit if RaceFacer's target printer (`$(".pos_data").data("termalprintername")` in its own page) turns out to differ from the one selected here — that would explain why the raw macros land on the right device for RaceFacer but not for us. The button is on `/session` (kiosk mode, F9) so a CSR can open the drawer without a supervisor login, and also on `/parametres` for testing.
