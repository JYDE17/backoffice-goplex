...
`openCashDrawer()` reuses that same QZ Tray connection to pop the cash drawer, since it's wired through the receipt printer's port rather than being its own USB device. It does **not** send generic ESC/POS bytes — this venue's printer/driver doesn't respond to those. The exact command was recovered by inspecting RaceFacer's own `window.open_drawer` in its `pos.js` (RaceFacer bundles its own copy of `qz-tray.js` and already kicks this same drawer successfully): two raw text macros, `"p\x0022"` and `"p22"`, sent as plain-string print jobs (no `type`/`format` object — QZ Tray defaults bare strings to raw/command). Both are sent every time, matching RaceFacer's own code, since only one is honored depending on the driver; each is caught independently so a rejection on one doesn't block the other. The button is on `/session` (kiosk mode, F9) so a CSR can open the drawer without a supervisor login, and also on `/parametres` for testing.
